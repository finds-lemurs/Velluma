...
function gasLink() {
  return `${NETWORK.explorer}/gastracker`;
}
function short(a) {
...
export async function run() {
  ...
  try {
    const chainIdHex = await readRpcHealth();
    console.log(`- eth_chainId result: ${chainIdHex ?? "null"}`);
  } catch (e) {
  ...
  console.log(`- Gas price (approx gwei): ${(Number(gasPrice) / 1e9).toFixed(3)}`);
  console.log(`- Gas tracker: ${gasLink()}`);
  console.log("");
